            ...
            <form action="{{ route('cart.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                    ...
                    <div class="relative w-full aspect-square overflow-hidden border border-gray-100 bg-gray-50 group">
                        ...
                        <div class="slider-track h-full" id="{{ $sliderId }}" data-current-index="0"
                            data-total="{{ $totalSlide }}" style="
                                display: flex;
                            ...
                            <div class="h-full flex-shrink-0" style="width: calc(100% / {{ $totalSlide }});"
                                data-index-foto="{{ $indexFoto }}">
                                <img src="{{ asset('images/' . $pathFoto) }}" class="w-full h-full object-cover"
                                    alt="{{ $p->nama }} - foto {{ $indexFoto + 1 }}" loading="lazy">
                            </div>
                            @empty
                            <div class="h-full flex-shrink-0" style="width: 100%;">
                                <img src="{{ asset('images/default.jpg') }}" class="w-full h-full object-cover"
                                    alt="No Image">
                            </div>
                            @endforelse
                        ...
                        @if($totalSlide > 1)
                        <button type="button" onclick="moveSlidePopup('{{ $sliderId }}', '{{ $dotClass }}', -1)"
                        ...
                        {{-- Dots indikator --}}
                        <div
                            class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 bg-black/20 px-2 py-1 rounded-full z-10 items-center">
                            @foreach($semuaFoto as $index => $pathFoto)
                            <span
                                class="{{ $dotClass }} block rounded-full transition-all duration-300 h-2 {{ $index === 0 ? 'w-4 bg-white opacity-100' : 'w-2 bg-white/50 opacity-50' }}"></span>
                            @endforeach
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <p class="text-[12px] text-gray-500 leading-relaxed mb-4 whitespace-pre-line">
                            {{ $p->deskripsi }}</p>

                        {{-- Pilih Variasi --}}
                        @php
                        $adaJenisVariasi = $p->variasis->contains(fn($v) => !empty($v->ukuran) || !empty($v->warna));
                        @endphp
                        @if($adaJenisVariasi)
<div class="mb-4">
    <span class="text-[10px] text-gray-800 block mb-2 font-bold uppercase tracking-widest">Pilih Variasi</span>
    <div class="flex flex-wrap gap-2">
        @foreach($p->variasis as $keyVar => $v)
        @php
        ...
        <label class="cursor-pointer">
            <input type="radio" name="produk_variasi_id" value="{{ $v->id }}"
                class="peer hidden radio-variasi" data-harga="{{ $v->harga }}"
                data-stok="{{ $v->stok }}" data-index-foto="{{ $fotoIndex }}"
                data-slider-id="{{ $sliderId }}" data-dot-class="{{ $dotClass }}"
                {{ $keyVar == 0 ? 'checked' : '' }}>
            <span class="block border rounded-md px-3 py-1.5 text-[11px] font-medium transition-colors
        {{ $isHabis
            ? 'border-red-400 text-red-400 peer-checked:bg-red-50'
            : 'border-gray-200 text-gray-600 peer-checked:border-[#001f3f] peer-checked:bg-blue-50/30' }}">
                {{ $v->ukuran }} {{ $v->warna }} {{ $isHabis ? '(Habis)' : '' }}
            </span>
        </label>
        @endforeach
    </div>
...
@else
    {{-- Produk tanpa pilihan ukuran/warna: tetap kirim variasi default supaya bisa masuk keranjang --}}
    <input type="radio" name="produk_variasi_id" value="{{ $p->variasis->first()->id ?? '' }}"
        class="hidden radio-variasi" data-harga="{{ $p->variasis->first()->harga ?? $p->harga }}"
        data-stok="{{ $p->variasis->first()->stok ?? 0 }}" data-index-foto="0"
        data-slider-id="{{ $sliderId }}" data-dot-class="{{ $dotClass }}" checked>
@endif
                        ...
                        <div class="mb-6">
                            <span
                                class="text-[10px] text-gray-800 block mb-2 font-bold uppercase tracking-widest">Jumlah</span>
            ...
            <button data-modal-hide="{{ $modalId }}" class="absolute top-4 right-4 text-gray-400 hover:text-red-500"
                aria-label="Tutup">
