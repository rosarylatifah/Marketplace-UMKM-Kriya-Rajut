...
<div class="py-12 max-w-6xl mx-auto px-4 lg:px-0">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden">

            {{-- Header: ID & Status --}}
            <div class="px-6 pt-6 pb-4 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-2">
                <p class="text-[10px] text-gray-400 font-mono uppercase tracking-widest">ID: {{ $pesanan->id_pesanan }}</p>
                <div class="flex items-center gap-2">
                    @if($pesanan->status === 'BELUM KONFIRMASI')
                        <div class="h-1.5 w-1.5 rounded-full bg-gray-400 animate-pulse"></div>
                        <p class="text-[10px] font-bold text-gray-500 italic tracking-widest uppercase">Menunggu Konfirmasi</p>
                    ...
                    @elseif($pesanan->status === 'PENGAJUAN BATAL')
                        ...
                        <p class="text-[10px] font-bold text-orange-500 italic tracking-widest uppercase">Menunggu Konfirmasi Pembatalan</p>
                    @else
            ...
            {{-- List Item Produk (per item, kayak keranjang) --}}
            <div class="p-6 space-y-4">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <div class="flex-grow">
                        <h3 class="font-bold text-xs text-[#001f3f] uppercase tracking-widest">{{ $item['nama'] }}</h3>
                        <p class="text-[10px] text-gray-400 mt-0.5">
                            IDR {{ number_format($item['harga'], 0, ',', '.') }} &times; {{ $item['quantity'] }}
                        </p>
                        <button type="button"
                    ...
                    <div class="text-right flex-shrink-0">
                        <p class="text-[9px] text-gray-400 uppercase tracking-widest">Subtotal</p>
                        <p class="font-bold text-[#001f3f] text-[11px]">
                            Rp {{ number_format($item['harga'] * $item['quantity'], 0, ',', '.') }}
                        </p>
                    </div>
                </div>
                @empty
                {{-- Fallback untuk pesanan lama yang belum punya items_snapshot --}}
                <div class="flex gap-4 items-center">
                    <div class="w-16 h-16 bg-gray-50 border border-gray-100 flex-shrink-0 flex items-center justify-center overflow-hidden rounded-lg">
                        <i class="fa-solid fa-box text-xl text-gray-300"></i>
                    </div>
                    <h3 class="font-bold text-xs text-[#001f3f] uppercase tracking-widest">{{ $pesanan->nama_barang }}</h3>
                </div>
                @endforelse
            </div>
            ...
            <div class="px-6 pb-6 pt-4 border-t border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    <a href="https://wa.me/6285778092881" target="_blank"
                        class="flex items-center justify-center gap-2 border border-gray-200 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest text-gray-600 hover:bg-gray-50 transition-all w-full md:w-auto text-center">
                    ...
                    @if($pesanan->status === 'BELUM KONFIRMASI')
                        <button type="button" onclick="konfirmasiPembatalan('{{ $pesanan->id_pesanan }}')"
                            class="flex items-center justify-center gap-2 border border-red-200 bg-red-50 hover:bg-red-100 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest text-red-600 transition-all w-full md:w-auto text-center cursor-pointer">
                            <i class="fa-solid fa-xmark text-xs"></i>
                            Ajukan Pembatalan
                        </button>

                    @elseif($pesanan->status === 'PENGAJUAN BATAL')
                        <div class="flex items-center justify-center gap-2 border border-orange-200 bg-orange-50 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest text-orange-600 w-full md:w-auto text-center">
                    ...
                    @elseif($pesanan->status === 'DALAM PERJALANAN')
                        <form action="{{ route('pesanan.konfirmasi', $pesanan->id) }}" method="POST"
                            ...
                            @csrf
                            <button type="submit"
                                class="flex items-center justify-center gap-2 border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest text-emerald-600 transition-all w-full md:w-auto text-center cursor-pointer">
                    ...
                    @elseif($pesanan->status === 'SELESAI')
                        <div class="flex items-center justify-center gap-2 border border-emerald-200 bg-emerald-50 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest text-emerald-600 w-full md:w-auto text-center">
                            <i class="fa-solid fa-circle-check text-xs"></i>
                            Pesanan Diterima
                        </div>
                        <button type="button" data-modal-target="modal-komplain" data-modal-toggle="modal-komplain"
                            class="flex items-center justify-center gap-2 border border-orange-200 bg-orange-50 hover:bg-orange-100 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest text-orange-600 transition-all w-full md:w-auto text-center cursor-pointer">
                            <i class="fa-solid fa-rotate-left text-xs"></i>
                            Ajukan Retur / Komplain
                        </button>
                    @endif
                </div>
            </div>
    ...
    <div class="mt-12 text-center">
        <a href="/katalog"
            class="bg-[#001f3f] text-white px-10 py-4 rounded-full text-[10px] font-bold tracking-[0.2em] shadow-lg hover:bg-gray-800 transition-all inline-flex items-center gap-3">
...
{{-- MODAL DETAIL (disamakan persis kayak di keranjang.blade.php) --}}
<div id="modal-detail"
    ...
    <div class="relative p-4 w-full max-w-lg max-h-full">
        <div class="relative bg-white rounded-xl shadow">
            <div class="flex items-center justify-between p-6 border-b border-gray-100">
                <h3 class="text-[11px] font-bold text-[#001f3f] uppercase tracking-[0.2em]">Ajukan Retur / Komplain</h3>
            ...
            <form id="form-komplain" onsubmit="kirimKomplain(event)" class="p-6 space-y-5">
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-[0.25em] text-gray-400 mb-2">Jenis Pengajuan</label>
                    <select id="komplain-jenis" required
                        class="w-full border-b-2 border-gray-100 border-t-0 border-l-0 border-r-0 focus:border-[#001f3f] focus:ring-0 text-[11px] py-3 px-0 font-bold uppercase tracking-widest bg-transparent outline-none">
                ...
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-[0.25em] text-gray-400 mb-2">Alasan / Penjelasan</label>
                    <textarea id="komplain-alasan" rows="3" required placeholder="Ceritakan kendalanya..."
                        class="w-full border-b-2 border-gray-100 border-t-0 border-l-0 border-r-0 focus:border-[#001f3f] focus:ring-0 text-[13px] py-3 px-0 bg-transparent outline-none resize-none"></textarea>
                </div>
                <p class="text-[10px] text-gray-400 italic">Kalau ada foto bukti, lampirkan langsung di chat WhatsApp setelah ini ya.</p>
                <button type="submit"
                    class="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1ebe57] text-white font-bold py-3.5 rounded-full uppercase text-[11px] tracking-[0.25em] transition-all duration-200 shadow-md hover:shadow-lg">
...
<script>
    ...
    function kirimKomplain(e) {
        ...
        const pesan = `Halo admin, saya mengajukan ${jenis} untuk pesanan ${idPesanan}. Alasan: ${alasan}`;
