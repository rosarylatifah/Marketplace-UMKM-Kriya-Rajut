...
<div class="py-8 max-w-7xl mx-auto px-4">
    <div class="mb-5 text-center">
        ...
        <p class="text-xs text-gray-400 mt-2 uppercase tracking-widest">— Koleksi Rajutan Tangan —</p>
    </div>
    ...
    <div class="max-w-md mx-auto mb-6 px-4">
        <form action="{{ url()->current() }}" method="GET"
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari produk rajutan..."
                class="w-full bg-transparent border-none p-0 text-xs font-medium uppercase tracking-wider text-gray-700 placeholder-gray-400 focus:ring-0">
            @if(request('search'))
            <a href="{{ route('katalog', request()->route('category') ?? 'semua') }}"
                class="text-gray-400 hover:text-red-500 mr-2 text-xs">✕</a>
            @endif
    ...
    {{-- NAV KATEGORI STICKY --}}
    <div id="nav-kategori" class="sticky z-[30] w-full transition-all duration-300">
        <div id="bg-kategori-wrapper"
            ...
            <div class="relative max-w-4xl mx-auto px-8 group/nav">
                ...
                <div id="container-kategori"
                    class="flex items-center justify-start md:justify-center gap-5 md:gap-8 overflow-x-auto whitespace-nowrap scroll-smooth no-scrollbar px-2"
                    style="-webkit-overflow-scrolling: touch;">
                    @php $isActiveSemua = !isset($currentCategory) || $currentCategory == 'Semua'; @endphp
                    <a href="{{ route('katalog') }}"
                        class="inline-block text-xs font-bold uppercase tracking-widest transition-all {{ $isActiveSemua ? 'text-[#001f3f] underline underline-offset-8 decoration-2' : 'text-gray-400 hover:text-[#001f3f]' }}">
                        Semua
                    </a>
                    @foreach($kategoris as $k)
                    @php $isActive = isset($currentCategory) && $currentCategory == $k->nama; @endphp
                    <a href="{{ route('katalog', $k->slug) }}"
                        class="inline-block text-xs font-bold uppercase tracking-widest transition-all {{ $isActive ? 'text-[#001f3f] underline underline-offset-8 decoration-2' : 'text-gray-400 hover:text-[#001f3f]' }}">
                        {{ $k->nama }}
                    </a>
                    @endforeach
                </div>
    ...
    <div class="max-w-4xl mx-auto mb-8 px-2">
        <form action="{{ url()->current() }}" method="GET" class="flex flex-wrap items-center justify-center gap-3">
            <input type="hidden" name="search" value="{{ request('search') }}">
            <div class="flex items-center gap-2">
                <label class="text-[11px] uppercase tracking-widest text-gray-400 font-bold">Urutkan</label>
                <select name="urutkan" onchange="this.form.submit()"
                    class="text-[11px] font-bold uppercase tracking-widest border border-gray-200 rounded-full px-4 py-2 outline-none focus:border-[#001f3f] bg-white text-gray-700 cursor-pointer">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <label class="text-[11px] uppercase tracking-widest text-gray-400 font-bold">Harga</label>
                <input type="number" name="harga_min" value="{{ request('harga_min') }}" placeholder="Min"
                    class="w-24 text-xs border border-gray-200 rounded-full px-4 py-2 outline-none focus:border-[#001f3f] bg-white text-gray-700 placeholder-gray-300">
                <span class="text-gray-300 text-xs">—</span>
                <input type="number" name="harga_max" value="{{ request('harga_max') }}" placeholder="Max"
                    class="w-24 text-xs border border-gray-200 rounded-full px-4 py-2 outline-none focus:border-[#001f3f] bg-white text-gray-700 placeholder-gray-300">
                <button type="submit"
                    class="bg-[#001f3f] hover:bg-[#003366] text-white text-xs font-bold uppercase tracking-widest px-5 py-2 rounded-full transition-all">
                    Terapkan
                </button>
            </div>
            @if(request('harga_min') || request('harga_max') || request('urutkan'))
            <a href="{{ url()->current() }}{{ request('search') ? '?search='.request('search') : '' }}"
                class="text-xs text-gray-400 hover:text-red-500 uppercase tracking-widest font-bold underline">
                Reset Filter
            </a>
            @endif
        </form>
    ...
    {{-- GRID PRODUK --}}
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-16 mt-4">
        ...
        <div
            ...
            <div
                class="w-full bg-gray-50 aspect-square mb-3 border border-gray-100 flex items-center justify-center overflow-hidden">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        modals.forEach(modal => {
            ...
            function setStickyOffset() {
                ...
                if (navbar && navKategori) {
                    navKategori.style.top = navbar.getBoundingClientRect().height + 'px';
                }
            }
    ...
    window.addEventListener('scroll', function () {
        ...
        if (window.scrollY >= sensor.offsetTop) {
